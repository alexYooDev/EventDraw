...
export function LandingPage() {
  ...
  const activeSlug = slug || 'didi-beauty'; // Fallback to 'didi-beauty' if no slug is provided (for root path)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const prizeData = await organizationService.getPublicPrizes(activeSlug);
        setOrg(orgData);
        // Sort prizes by place (1, 2, 3)
  ...
  // Get prizes in order: 2nd, 1st, 3rd for the podium
  const firstPrize = prizes.find(p => p.place === 1);
  const secondPrize = prizes.find(p => p.place === 2);
  const thirdPrize = prizes.find(p => p.place === 3);

  const podiumPrizes = [
    { 
      ...secondPrize, 
      displayPlace: '2nd Place', 
      color: 'gray', 
      height: 'h-64 sm:h-72', 
      order: 'order-1',
      featured: false
    ...
    { 
      ...thirdPrize, 
      displayPlace: '3rd Place', 
      color: 'orange', 
      height: 'h-56 sm:h-64', 
      order: 'order-3',
      featured: false
    }
  ].filter(p => p.id !== undefined); // Only show if they exist
    ...
    <div className={`min-h-screen w-full bg-gradient-to-br ${theme.colors.background}`}>
      ...
      <header className="bg-white shadow-md w-full">
        <div className="w-full px-4 py-6 sm:py-8">
          <h1 className={`text-3xl sm:text-5xl font-extrabold text-center bg-gradient-to-r ${theme.gradients.header} bg-clip-text text-transparent`}>
            {theme.emojis.main} {org?.name || 'Luck of a Draw'} X Luck of a Draw !
          </h1>
          <p className="text-center text-gray-600 mt-2 text-base sm:text-lg max-w-2xl mx-auto">
            Share your experience with us and you could win one of our premium rewards!
          </p>
        </div>
      </header>
      ...
      <footer className="bg-white mt-12 py-8 shadow-inner w-full">
        <div className="w-full px-4 text-center">
          <p className="text-gray-600 text-sm mb-4">© 2025 {org?.name || 'Luck of a Draw'}. All rights reserved.</p>
